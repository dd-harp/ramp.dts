...
#' @description Implements [parse_dts_out_X] for the SIP_dts model
#' @inheritParams parse_dts_out_X
#' @return none
#' @export
parse_dts_out_X.SIP_dts <- function(dts_out, pars, i) {
  time = dts_out[,1]
  with(pars$ix$X[[i]],{
    S = dts_out[,S_ix+1]
    I = dts_out[,I_ix+1]
...
#' @export
dts_plot_X.SIP_dts = function(pars, i=1, clrs=c("darkblue", "darkred", "darkgreen"), llty=1, stable=FALSE, add_axes=TRUE){
  ...
  dts_lines_X_SIP_dts(vars$XH[[i]], pars, clrs, llty)
}


#' Add lines for the density of infected individuals for the SIP_dts model
#'
#' @param XH a list with the outputs of parse_dts_out_X.SIP_dts
#' @param pars a list that defines an `ramp.dts` model (*e.g.*,  generated by `dts_setup()`)
#' @param clrs a vector of colors
#' @param llty an integer (or integers) to set the `lty` for plotting
#'
#' @export
dts_lines_X_SIP_dts = function(XH, pars, clrs=c("darkblue", "darkred", "darkgreen"), llty=1){
  with(XH,{
