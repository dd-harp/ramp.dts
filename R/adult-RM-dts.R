...
#' @export
setup_MYZpar.RM_dts = function(MYZname, pars, s, MYZopts=list(), EIPname, calK){
  pars$MYZpar[[s]] = make_MYZpar_RM_dts(pars$nPatches, MYZopts, EIPname, calK)
  return(pars)
}
...
#' @return a [list]
#' @export
make_MYZpar_RM_dts = function(nPatches, MYZopts=list(), EIPname, calK,
                          p=11/12,
                          sigma=1/8,
                          f=0.3,
                          q=0.95,
...
#' @return a [list]
#' @export
make_parameters_MYZ_RM_dts <- function(pars, EIPname, p, sigma, f, q, nu, eggsPerBatch, eip, calK) {
...
#' @description Implements [parse_dts_out_MYZ] for the RM_dts model
#' @inheritParams parse_dts_out_MYZ
#' @return a [list]
#' @export
parse_dts_out_MYZ.RM_dts <- function(dts_out, pars, s) {with(pars$ix$MYZ[[s]],{
  time = dts_out[,1]
  M = dts_out[,M_ix+1]
  P = dts_out[,P_ix+1]
  U = dts_out[,U_ix+1]
