...
#' @description Implements [parse_dts_out_L] for stages_dts competition model.
#' @inheritParams parse_dts_out_L
#' @return [list]
#' @export
parse_dts_out_L.stages_dts <- function(dts_out, pars, s) {
  time = dts_out[,1]
  L1 = dts_out[,pars$ix$L[[s]]$L1_ix+1]
  L2 = dts_out[,pars$ix$L[[s]]$L2_ix+1]
  L3 = dts_out[,pars$ix$L[[s]]$L3_ix+1]
  L4 = dts_out[,pars$ix$L[[s]]$L4_ix+1]
  P = dts_out[,pars$ix$L[[s]]$P_ix+1]
