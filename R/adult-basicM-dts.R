...
#' @export
setup_MYZpar.basicM_dts = function(MYZname, pars, s, MYZopts=list(), EIPname, calK){
  pars$MYZpar[[s]] = make_MYZpar_basicM_dts(pars$nPatches, MYZopts, EIPname, calK)
  return(pars)
}
...
#' @return a [list]
#' @export
make_MYZpar_basicM_dts = function(nPatches, MYZopts=list(), EIPname, calK,
...
#' @description Implements [parse_dts_out_MYZ] for the basicM_dts model
#' @inheritParams parse_dts_out_MYZ
#' @return a [list]
#' @export
parse_dts_out_MYZ.basicM_dts <- function(dts_out, pars, s) {with(pars$ix$MYZ[[s]],{
  time = dts_out[,1]
  M = dts_out[,M_ix+1]
  P = dts_out[,P_ix+1]
  parous = P/M
