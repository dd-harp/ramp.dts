...
#' @export
setup_MYZpar.ENBRQ_dts = function(MYZname, pars, s, MYZopts=list(), EIPname, calK){
...
#' @description Implements [parse_dts_out_MYZ] for the ENBRQ_dts model
#' @inheritParams parse_dts_out_MYZ
#' @return a [list]
#' @export
parse_dts_out_MYZ.ENBRQ_dts <- function(dts_out, pars, s) {with(pars$ix$MYZ[[s]],{
  time = dts_out[,1]
  E = dts_out[,E_ix+1]
  N = dts_out[,N_ix+1]
  B = dts_out[,B_ix+1]
